    ...
    public function LoginConfirmation()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($results) {
                ...
                $redirectUrl = 'http://localhost/AdminOperations/View/viewGameDetails.php';
                echo json_encode(['status' => 'success', 'message' => 'Login Successful', 'redirect' => $redirectUrl]);
                exit;
            } elseif ($adminResults) {
                $_SESSION['admin_id']=$adminObj->getId();
                $redirectUrl = 'http://localhost/AdminOperations/View/adminPanel.php';
                echo json_encode(['status' => 'success', 'message' => 'Login Successful', 'redirect' => $redirectUrl]);
                exit;
            } else {
                $redirectUrl = 'http://localhost/AdminOperations/View/login.php';
